...
export function CourseDetailHeader({
  ...
  const handleStartLearning = () => {
    try {
      const key = "myLearning";
      const raw =
        typeof window !== "undefined" ? window.localStorage.getItem(key) : null;
      const parsed = raw ? JSON.parse(raw) : [];
      const isArray = Array.isArray(parsed) ? parsed : [];
      const exists = isArray.some(
      ...
      if (!exists) {
        ...
        const next = [...isArray, newItem];
        window.localStorage.setItem(key, JSON.stringify(next));
        toast({ title: "Added to My Learning" });
      }
      router.push(`/elearning/learn/${course.id}/${firstLessonId}`);
    } catch (_) {
      router.push(
        `/elearning/learn/${course.id}/${
