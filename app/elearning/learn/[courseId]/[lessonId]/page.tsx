...
export default function LessonPlayerPage() {
  ...
  // Check completed status and minted status from storage/API
  // Initialize completed status from localStorage
  // Initialize minted status from /api/nft/minted
  ...
  useMemo(() => {
    try {
      ...
      const item = arr.find((it: any) => String(it?.id) === String(courseId));
    ...
    (async () => {
      try {
        ...
        const has = (j?.minted ?? []).some(
          (m: any) => String(m.courseId) === String(courseId)
        );
        setIsMinted(!!has);
      } catch (_) {
        // ignore
  ...
  function handleCompleteIn10s() {
    if (completing || isCompletedCourse) return;
    setCompleting(true);
    setTimeout(() => {
      try {
        const raw =
        ...
        const idx = arr.findIndex(
          (it: any) => String(it?.id) === String(courseId)
        );
        const updated = {
          id: String(courseId),
          progress: 100,
        ...
        window.localStorage.setItem("myLearning", JSON.stringify(arr));
        setIsCompletedCourse(true);
  ...
  async function handleMint() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({
          title: "NFT Minted",
          description: "Course NFT minted successfully. View it in your NFTs.",
        });
      } else if (res.status === 409) {
        setIsMinted(true);
        toast({
          title: "Already Minted",
          description: "You have already minted this course NFT.",
        });
      } else {
        toast({
          title: "Mint Failed",
          description: data?.error || "Unable to mint course NFT",
          variant: "destructive",
        });
      }
    } catch (e: any) {
      toast({
        title: "Mint Error",
        description: e?.message || "Unexpected error occurred during minting",
        variant: "destructive",
      });
    } finally {
      setIsMinting(false);
          ...
          <div className="aspect-video bg-black">
            <VideoPlayer
              ...
              onComplete={async () => {
                ...
                // If this is the final lesson of the course, mint the NFT
                if (!nextLesson && !isMinted && !isMinting) {
                  try {
                    setIsMinting(true);
                    const res = await fetch("/api/nft/mint", {
                      method: "POST",
                    ...
                    if (res.ok) {
                      ...
                      toast({
                        title: "NFT Minted",
                        description:
                          "Course NFT minted successfully. View it in your NFTs.",
                      });
                    } else if (res.status === 409) {
                      setIsMinted(true);
                      toast({
                    ...
                    toast({
                      ...
                      description:
                        e?.message ||
                        "Unexpected error occurred during minting",
                      variant: "destructive",
                    });
                  } finally {
                    setIsMinting(false);
            ...
            <div className="max-w-4xl">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {/* Complete course in 10s */}
                  {!isCompletedCourse && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={handleCompleteIn10s}
                      disabled={completing}
                    >
                      {completing ? "Completing..." : "Complete in 10s"}
                    </Button>
                  )}

                  {/* Mint NFT after completed */}
                  {isCompletedCourse && !isMinted && (
                    <Button
                      size="sm"
                      variant="secondary"
                      onClick={handleMint}
