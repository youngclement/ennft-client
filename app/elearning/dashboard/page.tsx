...
export default function LearningDashboardPage() {
    ...
    const enrolledCourses = mockCourses.slice(0, 3).map((course, index) => ({
        ...course,
        progress: [25, 60, 90][index],
        lastAccessed: new Date(Date.now() - index * 24 * 60 * 60 * 1000).toISOString(),
        status: index === 2 ? 'completed' : 'in-progress',
        nextLesson: course.sections[0]?.lessons.find(l => !['1', '2', '3'].includes(l.id))?.title || 'Continue Learning'
    }));

    const totalCourses = enrolledCourses.length;
    const completedCourses = enrolledCourses.filter(c => c.status === 'completed').length;
    const totalProgress = enrolledCourses.reduce((acc, course) => acc + course.progress, 0) / totalCourses;
    const totalStudyTime = enrolledCourses.reduce((acc, course) => acc + (course.duration * course.progress / 100), 0);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <Tabs defaultValue="courses" className="w-full">
                        ...
                        <TabsContent value="courses" className="space-y-6">
                            ...
                            <Card className="p-6">
                                ...
                                <ProgressBar value={(weeklyProgress / weeklyGoal) * 100} className="h-3" />
                            </Card>
                                        ...
                                        <div className="flex">
                                            ...
                                            <div className="flex-1 p-4">
                                                <div className="flex items-start justify-between mb-2">
                                                    ...
                                                    <Badge variant={course.status === 'completed' ? 'default' : 'secondary'} className="ml-2">
                                        ...
                                        <div key={index} className="flex items-start gap-4 pb-4 border-b border-border last:border-b-0 last:pb-0">
                                            ...
                                            <div className="flex-1">
                                                <p className="font-medium">
                                                    ...
                                                    {activity.type === 'course_enrolled' && `Enrolled in "${activity.courseTitle}"`}
                        ...
                        <TabsContent value="achievements" className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {achievements.map((achievement, index) => (
                                    <Card key={index} className={`p-6 ${achievement.earned ? 'border-primary/20 bg-primary/5' : 'opacity-75'}`}>
                                        <div className="flex items-center gap-4">
                ...
                <div className="space-y-6">
                    ...
                    <Card className="p-6">
                        ...
                        <div className="space-y-4">
                            {mockCourses.filter(course => !enrolledCourses.some(ec => ec.id === course.id)).slice(0, 2).map((course) => (
